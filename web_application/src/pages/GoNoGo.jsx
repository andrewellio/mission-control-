...
export default function GoNoGo() {
  ...
  const [
    rocketVelocityLaunchRailThreshold,
    setRocketVelocityLaunchRailThreshold,
  ] = useState("");
  ...
  return (
    <div style={styles.title}>
      ...
      <p>
        *Angle of attack after the rocket leaves the launch rail. <br /> High
        angle of attack - the angle of the airstream to the fins, can make a
        stable rocket, unstable.{" "}
      </p>
      <input
        type="int"
      ...
      <div>
        {launchRodAngleThreshold - 10 > launchRodAngleValue - 10 ? (
          <label style={styles.goStyle}>Go</label>
        ) : (
      ...
      <p>
        *Velocity of the rocket as it leaves the launch rail. <br /> Reported by
        openRocket and calculate the angle of attack yourself with some trig.{" "}
      </p>
      <input //lanuchLatitude is CSV name
        ...
        style={styles.inputBox}
        onChange={(event) =>
          setRocketVelocityLaunchRailThreshold(event.target.value)
        }
      />
      ...
      <div>
        {rocketVelocityLaunchRailThreshold - 10 >
        rocketVelocityLaunchRailValue - 10 ? (
          <label style={styles.goStyle}>Go</label>
        ) : (
          <label style={styles.noGoStyle}>No Go</label>
        )}
      ...
      <p>
        *Take into account the rocket and if it has relatively high thrust.{" "}
      </p>
      <input //lanuchLatitude is CSV name
        type="int"
      ...
      <div>
        {windSpeedThreshold - 10 > windSpeedValue - 10 ? (
          <label style={styles.goStyle}>Go</label>
        ) : (
          <label style={styles.noGoStyle}>No Go</label>
        )}
